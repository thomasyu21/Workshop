...
c.execute("SELECT * FROM roster") #gets table roster from db file
if len(c.fetchall()) == 0: #if length of list is 0, add data
...
c.execute("SELECT * FROM courses")#gets table roster from db file
if len(c.fetchall()) == 0: #if length of list is 0, add data
